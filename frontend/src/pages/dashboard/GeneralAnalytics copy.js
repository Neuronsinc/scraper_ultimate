...
export default function GeneralAnalytics() {
  const varlink = `${process.env.REACT_APP_APIBACKEND}/datos-mongo}`;
  ...
  const [multiplesValues, setmultiplesdValues] = useState([]);
  const valuescategorias = (event, values) => {
    setmultiplesdValues(values.map((value) => value.title));
  };
  console.log(multiplesValues); // ["Casas", "Apartamentos"]

  const [filtrotable, setDatatable] = useState([]);

  useEffect(() => {
    const filtrodata = async () => {
      const filtrodata = {
        categorias: multiplesValues
      };
      axios
        .post(`${process.env.REACT_APP_APIBACKEND}/table-filtro`, filtrodata)
        .then((response) => {
